/* Auto‑generated from breakout layout – DO NOT EDIT MANUALLY */
#pragma once
#include <SDL3/SDL_rect.h>
static constexpr int NUM_BRICK_STATE = 2;
inline constexpr SDL_FRect BALL_COORDS{457, 1136, 107, 103};
inline constexpr SDL_FRect PAD_COORDS{260, 1312, 505, 75};
// inline constexpr SDL_FRect RED_BRICK_COORDS[3] = {
//     {35, 180, 60, 130},
//     {117,  30, 60, 130},
//     {280,  30, 60, 130}
// };
// inline constexpr SDL_FRect GREEN_BRICK_COORDS[3] = {
//     {35, 520, 60, 130},
//     {35, 370, 60, 130},
// {280, 370, 60, 130},
//
// };
// inline constexpr SDL_FRect ORANGE_BRICK_COORDS[3] = {
//     {35, 860, 60, 130},
//     {115,  710, 60, 130},
//     {280,  710, 60, 130}
// };
// inline constexpr SDL_FRect YELLOW_BRICK_COORDS[2] = {
//     {70, 770, 60, 130},
//     {70, 1200, 60, 130}
// };
inline constexpr SDL_FRect RED_BRICK_COORDS[2] = {
    {70, 70, 135, 80},
    {70,  890, 135, 80}
};
inline constexpr SDL_FRect GREEN_BRICK_COORDS[2] = {
    {70, 570, 135, 80},
    {70, 1100, 135, 80}
};
inline constexpr SDL_FRect ORANGE_BRICK_COORDS[2] = {
    {70, 270, 135, 80},
    {70,  1005, 135, 80}
};
inline constexpr SDL_FRect YELLOW_BRICK_COORDS[2] = {
    {70, 770, 135, 80},
    {70, 1200, 135, 80}
};


/* ---------- NEW PADDLES ---------- */
inline constexpr SDL_FRect PAD_SHORT_COORDS{570, 1220, 192, 77};  // short paddle (right of ball)
inline constexpr SDL_FRect PAD_LONG_COORDS { 48, 1410, 720, 77};  // very long paddle (bottom)

/* ---------- NEW POWER-UPS ---------- */
inline constexpr SDL_FRect POWERUP_SHRINK{797, 1210, 74, 74};  // "> <" symbol, slot 1
inline constexpr SDL_FRect POWERUP_ENLARGE{877, 1210, 74, 74};  // "< >" symbol, slot 2
inline constexpr SDL_FRect POWERUP_COIN{797, 1290, 74, 74};  // coin/dollar symbol, slot 3
inline constexpr SDL_FRect POWERUP_4_COORDS{877, 1290, 74, 74};  // empty slot, rename later
inline constexpr SDL_FRect POWERUP_5_COORDS{797, 1370, 74, 74};  // empty slot, rename later
inline constexpr SDL_FRect POWERUP_6_COORDS{877, 1370, 74, 74};  // empty slot, rename later
